...
const TaskDetailPage = () => {
  ...
  if (isTaskOwner && task.assignedTo) {
    // If current user is the customer and task is assigned, chat with the tasker
    chatPartnerId = task.assignedTo._id;
    chatPartnerName = task.assignedTo.name;
  } else if (isTaskerAssigned) {
    // If current user is the assigned tasker, chat with the customer
    chatPartnerId = task.customer._id;
    chatPartnerName = task.customer.name;
  }
  
  // Determine if chat should be available
  const isChatAvailable = (task.status === 'assigned' || task.status === 'inProgress' || task.status === 'completionRequested') && 
                          (isTaskOwner || isTaskerAssigned) && 
                          chatPartnerId;
  
  // Determine if task completion section should be shown
  const showTaskCompletion = isTaskOwner && task.status === 'inProgress' ? false : (isTaskOwner || isTaskerAssigned) && 
